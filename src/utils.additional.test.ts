...
Deno.test("Additional tests for utility functions", (t) => {
    t.step("escape and unescape edge cases", (t) => {
        t.step("should handle null or undefined input for escape", () => {
            expect(escape(null as any)).to.equal("");
            expect(escape(undefined as any)).to.equal("");
        });

        t.step("should handle null or undefined input for unescape", () => {
            expect(unescape(null as any)).to.equal("");
            expect(unescape(undefined as any)).to.equal("");
        });

        t.step("should handle empty string", () => {
            expect(escape("")).to.equal("");
            expect(unescape("")).to.equal("");
        });
    });

    t.step("additional seq tests", (t) => {
        t.step("should handle 0 length sequences", () => {
        ...
        t.step("should handle floating point values", () => {
            expect(seq(0, 3, 0.5)).to.deep.equal([0, 0.5, 1, 1.5, 2, 2.5]);
        });

        t.step("should return correct result for large ranges", () => {
