...
Deno.test("tests for html``", (t) => {
    t.step("should escape parameters", () => {
    ...
    t.step("should enable custom joiners", () => {
        const transform = (itm: number) => html`<li>${itm.toString()}</li>`;
    ...
    t.step("should allow empty custom joiner", () => {
        const transform = (itm: number) => html`<li>${itm.toString()}</li>`;
    ...
    t.step("should not erase zeroes", () => {
        const s = html`1${0}1`;
        expect(s).to.equal("101");
    });

    t.step("should handle booleans correctly", () => {
        const s = html`1${true}1`;
        expect(s).to.equal("1true1");
    });
});

Deno.test("Tests for mustache", (t) => {
    t.step("should substitute data that is present", () => {
        expect(
    ...
    t.step("should ignore spaces surrounding mustache names", () => {
        expect(
    ...
    t.step("should ignore escaped mustaches", () => {
        expect(mustache("\\{{ name }}", { name: "John" })).to.equal("{{ name }}");
    });

    t.step("should ignore absent mustaches", () => {
        expect(mustache("{{ nonexistent }}", { name: "John" })).to.equal(
            "{{ nonexistent }}",
        );
    });

    t.step("should perform replacements at once", () => {
        expect(
            mustache("Welcome to {{ location }}, {{ name }}.", {
                location: "{{ name }}",
                name: "John",
    ...
    t.step("should escape substituted text", () => {
        expect(
            mustache("Welcome to {{ location }}, {{ name }}.", {
                name: "John",
    ...
    t.step("Unescaped variables", (t) => {
        t.step("should not escape triple-braced variables", () => {
        ...
        t.step("should handle absent triple-braced variables", () => {
    ...
    t.step("Sections", (t) => {
        t.step("should render section when value is truthy", () => {
            expect(mustache("{{#show}}This is visible{{/show}}", { show: true }))
                .to.equal("This is visible");
        });

        t.step("should not render section when value is falsy", () => {
            expect(mustache("{{#show}}This is not visible{{/show}}", { show: false }))
                .to.equal("");
        });

        t.step("should render inverted section when value is falsy", () => {
            expect(mustache("{{^show}}This is visible{{/show}}", { show: false }))
                .to.equal("This is visible");
        });

        t.step("should not render inverted section when value is truthy", () => {
            expect(mustache("{{^show}}This is not visible{{/show}}", { show: true }))
                .to.equal("");
        });

        t.step("should treat empty arrays as falsy for inverted sections", () => {
            expect(mustache("{{^items}}No items{{/items}}", { items: [] }))
                .to.equal("No items");
        });
    });

    t.step("Array iteration", (t) => {
        t.step("should iterate over arrays with separators in template", () => {
        ...
        t.step("should use the dot value for primitive array items", () => {
    ...
    t.step("Context changes", (t) => {
        t.step("should change context inside sections with object values", () => {
    ...
    t.step("Nested sections", (t) => {
        t.step("should render nested sections correctly", () => {
            expect(mustache(
        ...
        t.step("should handle deep nesting", () => {
            expect(mustache(
                "{{#a}}A{{#b}}B{{#c}}C{{/c}}{{/b}}{{/a}}",
    ...
    t.step("Error cases", (t) => {
        t.step("should throw an error for unclosed sections", () => {
            expect(() => mustache("{{#section}}content", {})).to.throw(Error);
        });

        t.step("should throw an error for unopened section closers", () => {
            expect(() => mustache("content{{/section}}", {})).to.throw(Error);
        });
    });
});

Deno.test("Tests for template", (t) => {
